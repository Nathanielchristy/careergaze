...
import { 
  ...
  ShieldCheck, Download, PlusCircle, ClipboardList, 
  Clock
} from 'lucide-react'
...
export default function StudentTaskPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0A4D68] flex flex-col items-center justify-center text-white">
      ...
      <p className="text-xs font-bold tracking-widest uppercase opacity-50 italic">Loading Task Manager...</p>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <aside className={`fixed left-0 top-0 h-full w-72 bg-[#0A4D68] text-white flex flex-col p-6 z-[70] transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:w-64`}>
        ...
        <nav className="space-y-2 flex-1">
          ...
          <SidebarItem icon={<ClipboardList size={20}/>} label="Active Tasks" />
          <SidebarItem icon={<School size={20}/>} label="Colleges" />
        </nav>

        <div className="mt-auto pt-6 border-t border-white/10">
          <SidebarItem icon={<LogOut size={20} />} label="Logout" onClick={() => router.push('/login')} />
        </div>
      </aside>
      ...
      <main className="transition-all duration-300 lg:ml-64 p-4 md:p-8 lg:p-12">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start gap-4 mb-10">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-[#0A4D68]">
              Assign <span className="text-[#86C232]">Tasks</span>
            </h1>
            <p className="text-slate-500 text-sm font-semibold mt-1">
              Select an intern to deploy new project modules.
            </p>
          </div>
        ...
        <div className="relative mb-8">
          ...
          <input 
            ...
            className="w-full h-16 pl-14 pr-6 bg-white border-2 border-slate-100 rounded-2xl focus:outline-none focus:border-[#86C232] transition-all font-bold text-black shadow-sm"
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-100 border-b border-slate-200">
                  <th className="px-8 py-5 text-[11px] font-black uppercase text-black tracking-widest">Candidate</th>
                  <th className="px-8 py-5 text-[11px] font-black uppercase text-black tracking-widest">Track</th>
                  <th className="px-8 py-5 text-[11px] font-black uppercase text-black tracking-widest text-center">Work Status</th>
                  <th className="px-8 py-5 text-[11px] font-black uppercase text-black tracking-widest text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filtered.map((s, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-all cursor-default">
                    <td className="px-8 py-6">
                      <p className="font-bold text-black text-[15px]">{s.fullName}</p>
                      <p className="text-[11px] font-bold text-slate-400 italic">{s.college}</p>
                    </td>
                    <td className="px-8 py-6">
                      <span className="text-[10px] font-black bg-[#86C232] text-black px-2.5 py-1 rounded-md uppercase tracking-tighter">
                        {s.track}
                      </span>
                    </td>
                    <td className="px-8 py-6">
                       <div className="flex justify-center items-center gap-1.5 text-[10px] font-black uppercase text-orange-500">
                          <Clock size={12}/> Idle
                       </div>
                    </td>
          ...
          <>
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-[11px] font-black uppercase text-black mb-2 block">Task Category</label>
                ...
                <div>
                  <label className="text-[11px] font-black uppercase text-black mb-2 block">Project Deadline</label>
                ...
                <div>
                  <label className="text-[11px] font-black uppercase text-black mb-2 block">Module Instructions</label>
                  <textarea rows={5} className="w-full border-2 border-slate-100 rounded-xl p-4 font-bold text-black focus:border-[#86C232] outline-none placeholder:font-normal" placeholder="Outline the project scope..."></textarea>
                </div>

                <Button className="w-full h-14 bg-[#0A4D68] text-[#86C232] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-[#0A4D68]/20">
