...
import {
  ...
  TrendingUp, Menu, X, RefreshCw
} from 'lucide-react'
import { Card } from '@/components/ui/card'
import { Button } from '@/components/ui/button'

// YOUR NEW DEPLOYED LINK
...
export default function AdminDashboard() {
  ...
  const fetchSheetData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        pendingStudents: data.pendingStudents || 0, // Ensure your Apps Script sends this
        growthRate: 12 // Keeping this static for now
      })
    } catch (error) {
  ...
  if (!isAuthorized || loading) {
    return (
      <div className="min-h-screen bg-[#0A4D68] flex flex-col items-center justify-center text-white">
        ...
        <p className="text-xs font-bold tracking-widest uppercase opacity-50 italic">Syncing with Google Sheets...</p>
      </div>
    )
  }
  ...
  <SidebarItem 
    icon={<Users size={20}/>} 
    label="Students" 
    active={false} // Since we are navigating away
  />
</Link>
        ...
        <div className="mt-auto pt-6 border-t border-white/10">
          <div className="flex items-center gap-3 p-3 mb-4 bg-white/5 rounded-2xl border border-white/5">
            <div className="h-10 w-10 rounded-full bg-[#86C232] flex items-center justify-center font-bold text-[#0A4D68] shrink-0">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm flex items-center gap-2">
              Live Data Feed 
              <button onClick={fetchSheetData} className={`hover:text-[#86C232] transition-all ${refreshing ? 'animate-spin' : ''}`}>
                <RefreshCw size={14} />
              </button>
            </p>
          </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }} 
                    animate={{ width: `${stats.growthRate * 4}%` }} 
              ...
              <div className="grid grid-cols-3 gap-4 pt-8 border-t border-slate-50">
                ...
                <DetailItem label="Top College" value="Dynamic Data" />
  ...
  return (
    <Card className="p-4 md:p-6 rounded-2xl border-none shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center bg-white gap-2">
      <div>
        ...
        <p className="text-xl md:text-2xl font-black text-[#0A4D68] leading-none">
